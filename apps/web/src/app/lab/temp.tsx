    ...
    useEffect(() => {
        ...
        const loadGraphics = async () => {
            ...
            const THEME = {
                ...
                line1: 0x2ef2c8,
                line2: 0x86a8ff,
                asset: 0xdde3dd, // Light green-gray
                cable: 0x2ef2c8,
                accent: 0x2ef2c8,
                outlines: 0xffffff,
                tints: {
            ...
            let currentNoiseFunc: ((x: number, y: number) => number) | null = null;
            ...
            let seededTerrain: any = null;
            const updateSeededTerrain = (newSeed: string, hRange: number) => {
                if (!scene) return;
                if (seededTerrain) {
                    scene.remove(seededTerrain);
                    if (seededTerrain.geometry) seededTerrain.geometry.dispose();
                }

                currentNoiseFunc = createSeededNoise(newSeed);
                ...
                for (let i = 0; i < pos.count; i++) {
                    const x = pos.getX(i);
                    const y = pos.getY(i);
                    pos.setZ(i, currentNoiseFunc(x, y) * hRange);
                }
                ...
                seededTerrain = new THREE.Mesh(geo, mat);
                seededTerrain.rotation.x = -Math.PI / 2;
                seededTerrain.position.y = -110;
                scene.add(seededTerrain);
                ...
                // Re-create cables to follow new height
                createInfrastructureCables();
            };
            ...
            const getTerrainY = (x, z) => {
                if (currentNoiseFunc) {
                    return (currentNoiseFunc(x, z) * heightRange) - 110;
                }
